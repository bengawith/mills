...
# Provides thread-safe dispatching for real-time updates to clients.
# ---
...
# EventDispatcher class: provides thread-safe, async event dispatching for WebSocket notifications
class EventDispatcher:
    """
    Dispatches events to WebSocket clients in a thread-safe manner.
    Used to trigger real-time updates from synchronous service code.
    """
    def __init__(self):
        # ThreadPoolExecutor for running tasks in background threads if needed
        self.executor = ThreadPoolExecutor(max_workers=2)
        self.loop = None

    def get_or_create_event_loop(self):
        """
        Get the current event loop or create a new one if not present.
    ...
    def dispatch_maintenance_alert(self, ticket_id: int, machine_id: str, priority: str, description: str):
        """
        Dispatch a maintenance alert event to WebSocket clients.
        Schedules the notification coroutine in the event loop or runs it directly.
        """
        try:
    ...
    def dispatch_ticket_status_change(self, ticket_id: int, old_status: str, new_status: str, machine_id: str):
        """
        Dispatch a ticket status change event to WebSocket clients.
        Notifies clients of status transitions for maintenance tickets.
        """
        try:
    ...
    def dispatch_machine_status_change(self, machine_id: str, status: str, utilization: Optional[float] = None):
        """
        Dispatch a machine status change event to WebSocket clients.
        Notifies clients of changes in machine activity or utilization.
        """
        try:
    ...
    def dispatch_ticket_created(self, ticket: dict):
        """
        Dispatch a ticket created event to WebSocket clients.
        Notifies clients when a new maintenance ticket is created.
        """
        try:
    ...
    def dispatch_dashboard_refresh(self):
        """
        Trigger a dashboard refresh event for all WebSocket clients.
        Used to notify clients to reload dashboard data.
        """
        try:
...
# Global event dispatcher instance for use throughout the backend
