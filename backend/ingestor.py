...
def ingest_csv_data():
    ...
    # No longer filtering dates here; the full CSV data will be ingested.
    # The API fetching logic will handle dynamic date ranges.
...
def fetch_and_ingest_fourjaw_data():
    ...
    for machine_id in Config.MACHINE_IDS:
        ...
        if latest_db_timestamp:
            ...
            api_fetch_start_date = datetime(2025, 1, 1, tzinfo=timezone.utc) # Default if no data in DB
        ...
        if all_entries_for_machine:
            ...
            new_data_df = pd.DataFrame(all_entries_for_machine)
            ...
            if os.path.exists(csv_file_path):
                ...
                # Remove duplicates based on 'id' if they exist (from API and CSV overlap)
